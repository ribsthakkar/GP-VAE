...
frame = 4
total_frames = 8
experiment_imputed = 'models/200506_09_35_52_mixed-hgp-final/'
...
if validation_type == 'hmnist':
    ...
    corrupted[0].save(experiment_imputed+'/Corrupted' + str(sample) + '.gif', save_all=True, append_images=corrupted[1:], optimize=False, duration=100, loop=0)
    clean[0].save(experiment_imputed+'/Clean' + str(sample) + '.gif', save_all=True, append_images=clean[1:], optimize=False, duration=100, loop=0)
elif validation_type == 's2s-debug':
    s_data = np.load(spdir)
    imputed_data = np.load(experiment_imputed + 'imputed.npy')
    x_val_full = s_data['x_train_full'][8000:]
    x_val_miss = s_data['x_train_miss'][8000:]
    m_val_miss = s_data['m_train_miss'][8000:]
    # data = x_val_full[sample, frame].reshape(*shape)
    # mask = x_val_miss[sample, frame].reshape(*shape)
    # miss = m_val_miss[sample, frame].reshape(*shape)
    # imp = imputed_data[sample, frame].reshape(*shape)
    ...
    # axes[1, 0].imshow(imp)
    # axes[1, 1].imshow(miss)
    imputed = list(map(lambda x: Image.fromarray((x * 255).astype('uint8')),
                       np.clip(imputed_data[sample, :].reshape(total_frames, *shape), 0, 255)))
    corrupted = list(map(lambda x: Image.fromarray((x * 255).astype('uint8')),
                         np.clip(x_val_miss[sample, :].reshape(total_frames, *shape), 0, 255)))
    clean = list(map(lambda x: Image.fromarray((x * 255).astype('uint8')),
                     np.clip(x_val_full[sample, :].reshape(total_frames, *shape), 0, 255)))
    imputed[0].save(experiment_imputed + '/Imputed' + str(sample) + '.gif', save_all=True, append_images=imputed[1:],
                    optimize=False, duration=100, loop=0)
    corrupted[0].save(experiment_imputed + '/Corrupted' + str(sample) + '.gif', save_all=True,
                      append_images=corrupted[1:], optimize=False, duration=100, loop=0)
    clean[0].save(experiment_imputed + '/Clean' + str(sample) + '.gif', save_all=True, append_images=clean[1:],
                  optimize=False, duration=100, loop=0)
elif validation_type == 'b2b-debug':
    h_data = np.load(hmnistdir)
    s_data = np.load(spdir)
    imputed_data = np.load(experiment_imputed + 'imputed.npy')
    x_val_full = np.concatenate((h_data['x_train_full'][8000:], s_data['x_train_full'][8000:]))
    x_val_miss = np.concatenate((h_data['x_train_miss'][8000:], s_data['x_train_miss'][8000:]))
    m_val_miss = np.concatenate((h_data['m_train_miss'][8000:], s_data['m_train_miss'][8000:]))
    # data = x_val_full[sample, frame].reshape(*shape)
    # mask = x_val_miss[sample, frame].reshape(*shape)
    # miss = m_val_miss[sample, frame].reshape(*shape)
    # imp = imputed_data[sample, frame].reshape(*shape)
